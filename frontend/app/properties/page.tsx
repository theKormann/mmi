...
// Definindo o tipo de dado que vem da API
type Property = {
  id: number;
  title: string;
  price: number;
...
// Componente Skeleton para o estado de carregamento do PropertyCard
...
export default function HomePage() {
  ...
  // O restante dos dados estáticos da página continua igual
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <aside className="hidden md:block w-56 shrink-0">
            <div className="space-y-6 sticky top-24">
              <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/80">
                ...
                <ul className="space-y-2">
                  {["Apartamento", "Casa", "Cobertura", "Terreno", "Comercial"].map((type, index) => (
                    <li key={index}>
                      <Link href="#" className="flex items-center justify-between text-slate-600 hover:text-blue-600 transition-colors duration-300 group">
                        <span>{type}</span>
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-slate-200/80">
                ...
                <div className="flex flex-wrap gap-2">
                  {["Piscina", "Churrasqueira", "Condomínio Fechado", "Perto do Metrô", "Varanda Gourmet"].map(
                    (tag, index) => (
                      <Badge key={index} className="bg-slate-200 text-slate-700 hover:bg-blue-500 hover:text-white transition-colors duration-300 cursor-pointer">
                        {tag}
                      </Badge>
                    )
                  )}
          ...
          <main className="flex-1">
            ...
            <section className="mb-10">
              <div className="flex items-center justify-between mb-6">
                ...
                <a href="/properties/reall">
                  <Button variant="outline" className="border-slate-300 hover:bg-slate-100 hover:border-blue-500 transition-all duration-300">
                    Ver Todos
                  </Button>
                </a>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                {!loading && !error && featuredProperties.length === 0 && (
                  <p className="col-span-full text-center text-slate-600">Nenhum imóvel em destaque encontrado no momento.</p>
                )}
            ...
            <section className="mb-10">
              <h2 className="text-2xl font-bold flex items-center mb-6">
                <MessageSquareQuote className="mr-2 h-5 w-5 text-blue-600" />
                O Que Nossos Clientes Dizem
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {testimonials.map((testimonial, index) => (
                  <div key={index} className="bg-white/60 backdrop-blur-sm rounded-xl p-6 border border-slate-200/80 flex flex-col">
                    <div className="flex items-center mb-4">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          className={`h-5 w-5 ${i < testimonial.rating ? "text-amber-400 fill-amber-400" : "text-slate-300"}`}
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500">{testimonial.details}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {virtualTours.map((stream, index) => (
                  <div key={index} className="group relative">
                    <div className="bg-white/60 backdrop-blur-sm border border-slate-200/80 rounded-lg overflow-hidden transition-all duration-300 group-hover:border-blue-500 group-hover:shadow-lg group-hover:shadow-blue-500/10">
                      ...
                      <div className="p-4">
                        <h3 className="font-semibold mb-1 group-hover:text-blue-600 transition-colors duration-300">{stream.title}</h3>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-slate-500">{stream.agent}</span>
                        </div>
                      </div>
                    </div>
                  </div>

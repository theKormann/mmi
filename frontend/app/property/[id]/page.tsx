...
// Carrossel de imagens: Agora constrói a URL completa
const ImageCarousel = ({ images, title }: { images: string[]; title:string }) => {
  ...
  // Garante que haja pelo menos uma imagem placeholder
  const safeImages = images && images.length > 0 ? images : ["/placeholder.svg"]

  if (safeImages.length <= 1) {
    // Constrói a URL completa para a única imagem
    const singleImageUrl = safeImages[0].startsWith('/') ? `${API_URL}${safeImages[0]}` : safeImages[0];
    return (
  ...
  return (
    <div className="relative aspect-video lg:h-[500px] w-full overflow-hidden">
      <AnimatePresence initial={false}>
        ...
        >
          <Image
            src={`${API_URL}${safeImages[currentIndex]}`}
...
export default function PropertyDetail() {
  ...
  // ✅ **NOVO ESTADO: Armazena a lista de imagens para o carrossel**
    ...
    if (id) {
      fetch(`${API_URL}/api/properties/${id}`)
        ...
        .then((data: Property) => {
          ...
          // ✅ **MODIFICAÇÃO: Constrói a lista de imagens para o carrossel**
          const allImages = [];
          ...
          // 2. Adiciona as imagens da galeria
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      >
        <div className="max-w-6xl mx-auto bg-white/80 backdrop-blur-sm shadow-lg rounded-2xl overflow-hidden">
          {/* ✅ **Usa a nova lista de imagens combinadas** */}

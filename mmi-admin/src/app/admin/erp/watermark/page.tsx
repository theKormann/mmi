...
import { useState } from 'react' // Import 'DragEvent'
...
// --- CORREÇÃO 1: Voltei para a URL base. ---
// O seu código anterior estava com '.../api', o que causaria /api/api/v1...
const API_BASE_URL =
...
export default function WatermarkGeneratorPage() {
  ...
  const [isDragging, setIsDragging] = useState(false) // <-- NOVO: Estado para visual

  // --- NOVO: Função centralizada para processar o arquivo ---
  const processFile = (file: File) => {
    // Validação básica de tipo (opcional, mas recomendado)
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      processFile(file) // <-- USA A FUNÇÃO NOVA
    }
  }

  // --- NOVO: Handlers de Drag and Drop ---
  ...
  const handleDrop = (e: React.DragEvent<HTMLLabelElement>) => {
    e.preventDefault()
    setIsDragging(false)
    const file = e.dataTransfer.files?.[0] // Pega o arquivo do "drop"
    if (file) {
      processFile(file) // <-- USA A FUNÇÃO NOVA
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // --- CORREÇÃO 2: Endpoint completo correto ---
    ...
    <div className="container mx-auto px-4 py-10 min-h-screen">
      ...
      <div className="bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-100 max-w-5xl mx-auto">
        <form onSubmit={handleSubmit}>
          {/* --- CORREÇÃO 3: Adicionados eventos de Drag and Drop e classe dinâmica --- */}
          <label
            htmlFor="file-upload"
            className={`flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer transition-colors mb-6 ${
              isDragging
                ? 'border-indigo-600 bg-indigo-50' // Estilo quando arrastando
                : 'border-gray-300 bg-gray-50 hover:bg-gray-100' // Estilo padrão
            }`}
            onDragOver={handleDragOver} // <-- NOVO
            onDragLeave={handleDragLeave} // <-- NOVO
            onDrop={handleDrop} // <-- NOVO
          >
            {originalPreview ? (
              <img
                src={originalPreview}
        ...
        {/* O resto do componente (Erro e Resultado) permanece o mesmo */}
        {error && (
